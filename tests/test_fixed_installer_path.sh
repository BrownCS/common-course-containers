...
if ! grep -q '/usr/local/share/ccc/course_installer.sh' "$repo_root/lib/open.sh"; then
  echo "FAIL: open flow does not use a fixed installer path" >&2
  exit 2
fi

if ! grep -q '/usr/local/share/ccc/course_installer.sh' "$repo_root/Dockerfile.template"; then
